...
function hiddenDiary(){
    let div = document.querySelector('.diary')
    let isCollapsed = div.dataset.collapsed === "true"
    let startHeight = isCollapsed ? 60 : div.scrollHeight
    let endHeight = isCollapsed ? div.scrollHeight : 60
    let startTime
    if(isCollapsed){
        document.querySelector('.diary-info__hidden').innerText = 'Скрыть'
    }else{
        document.querySelector('.diary-info__hidden').innerText = 'Показать'
    }
